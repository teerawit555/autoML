...
def pick_dual_thresholds(
    ...
    *,
    ...
    # soft (T_low)
    min_precision_low: float = 0.990,   # ✅ เพิ่ม
    ...
    if len(cand_high) > 0:
        ...
        thr_high = float(row_high["thr"])
        reason_high = "high=fallback"

    # ---- low ----
    s_low = s[s["thr"] <= thr_high].copy()
    cand_low = s_low[
        (s_low["fast_precision"] >= min_precision_low) &   # ✅ ใช้ precision_low
        (s_low["fast_fp_rate"] <= max_fp_rate_low)
    ].copy()

    if len(cand_low) > 0:
        # เลือก threshold “ต่ำสุด” ที่ยังผ่านเงื่อนไข → เพื่อให้เกิด MAYBE zone
        row_low = cand_low.sort_values(
            ["thr", "fast_fp_rate", "mae_all"],
            ascending=[True, True, True],
        ).iloc[0]
        thr_low = float(row_low["thr"])
        reason_low = "low=lowest_thr_with_precision&fp_caps"
    else:
        thr_low = thr_high
        reason_low = "low=none_under_high"

    thr_low = float(min(thr_low, thr_high))
    return thr_high, thr_low, f"{reason_high} | {reason_low}"
...
def train(
    ...
    ZERO_DROP_EXTRA = [
        ...
        "band_width",
        "band_to_p2p",
        "tail_noise_ratio",  # optional
        "t_est_settle_ms",   # optional (แต่ระวังมันช่วยแฝง)
    ]
    ...
    # Stage A: อนุญาตใช้ band/enter เพื่อกัน FP-fast
    STAGE1_ALLOW = "all"  # หรือจะลิสต์เองก็ได้

    # Stage B: ห้ามใช้ feature ที่เป็นเวลาเข้า band หรือแฝงเวลาเข้า band
    REG_DROP_EXTRA = [
        # enter/band-time direct
        "t_enter_band_ms",
        "t_enter_norm",
        "enter_margin_ms",

        # enter/band-time derived (แฝง t_enter)
        "enter_vs_settle",
        "enter_minus_est",

        # (ถ้ามึงมีเพิ่มในอนาคต) in-band time variants
        "t_first_inband_ms",
        "t_last_outband_ms",
    ]
    ...
    if "t_enter_band_ms" not in df_slow.columns:
        raise ValueError("Need t_enter_band_ms in features to train residual regressor")

    df_slow["extra_ms"] = (df_slow[label].astype(float) - df_slow["t_enter_band_ms"].astype(float)).clip(lower=0.0)
    df_slow["extra_time_log"] = np.log1p(df_slow["extra_ms"].astype(float))
    ...
    # ✅ สำคัญ: drop feature ที่ไม่อยากให้ regressor ใช้
    df_reg_for_fit = df_reg_for_fit.drop(columns=REG_DROP_EXTRA, errors="ignore")
    df_reg_for_fit = df_reg_for_fit.drop(columns=["extra_ms"], errors="ignore")

    reg_feature_cols = [c for c in df_reg_for_fit.columns if c != "extra_time_log"]
    ...
    X_reg_in = X_reg_in.drop(columns=REG_DROP_EXTRA, errors="ignore")  # ✅ เพิ่ม
    X_reg_in = align_columns(X_reg_in, reg_feature_cols)
    # baseline enter time (ต้องมีใน X_feat/df_train)
    t_enter = X_feat["t_enter_band_ms"].to_numpy(dtype=float) if "t_enter_band_ms" in X_feat.columns else np.zeros(len(X_feat))

    extra_log_pred = wait_reg.predict(X_reg_in)
    extra_pred = np.expm1(np.asarray(extra_log_pred, dtype=float))
    extra_pred = np.clip(extra_pred, 0, None)

    wait_pred = t_enter + extra_pred
    ...
    thr_high, thr_low, dual_reason = pick_dual_thresholds(
        scan,
        min_precision_high=0.999,
        max_fp_rate_high=0.0002,
        min_precision_low=0.995,
