...
def derive_sanity_gate_from_train(
    ...
    *,
    ...
    thr_low: float,
    q_feat: float = 0.98,
    q_reg_fp: float = 0.95,   # ✅ เข้มขึ้นมากกว่าก่อน (กันเคส reg หลุดใหญ่)
) -> dict:
    # global true-fast (ไม่ต้อง near)
    ...
    out: dict[str, Any] = {
        ...
        "notes": f"Derived from train true-fast & near(thr_low={thr_low:.6f}); q_feat={q_feat}, q_reg_fp={q_reg_fp}. Used only for SOFT zone.",
    }
    ...
    # ✅ NEW: tail_std (ถ้ายังไม่นิ่ง tail จะปั่น)
    f_tail_std = "tail_std"
    ...
    # ✅ NEW: late_activity (ปลายช่วงท้ายยังมี movement)
    ...
    # ✅ NEW: reg veto (FP firewall) — derive จาก reg_pred ของ true-fast ใกล้ขอบ
    reg_vals = wait_pred[mask]
    reg_fp = _safe_quantile(reg_vals, q_reg_fp, DEFAULTS["reg_fp_ms"])
    # hard clamp กันหลุดแบบ 6ms แบบที่มึงเจอ
...
def apply_dual_threshold_with_sanity(
    *,
    proba_is_fast: np.ndarray,
    wait_pred: np.ndarray,      # <-- ใช้อันนี้แหละ ไม่ต้องไปหาคอลัมน์
    ...
    # default = ผ่าน
    sanity_ok = np.ones_like(is_fast_soft, dtype=bool)
    ...
    # --- optional: tail_std / late_activity ---
